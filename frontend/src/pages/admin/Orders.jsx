            ...
            {orders.map((order) => (
              <div key={order._id} className="bg-white rounded-2xl shadow p-6">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="font-bold text-pizza-dark">
                  ...
                  <div className="text-right">
                    <p
                      className={`font-semibold text-sm ${statusColor[order.status]}`}
                    >
                      {STATUS_LABELS[order.status] || order.status}
                    </p>
                    <p className="font-bold text-pizza-red">
                      ₹{order.totalPrice}
                    </p>
                  </div>
                </div>
                <div className="mt-3 text-sm text-gray-600">
                  {order.presetName ? (
                    <p>
                      🍕 <span className="font-medium">{order.presetName}</span>{" "}
                      (Preset Order)
                    </p>
                  ) : (
                    <p>
                      🍕 {order.pizza?.base?.name} + {order.pizza?.sauce?.name}{" "}
